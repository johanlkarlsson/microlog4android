...
public class DefaultLoggerRepositoryTest {
	...
	public void testSetLevelExistingLogger() {
		Logger logger = new Logger(DefaultLoggerRepositoryTest.class.getName());
		defaultLoggerRepository.addLogger(logger);
		defaultLoggerRepository.setLevel(DefaultLoggerRepositoryTest.class.getName(), Level.INFO);
		
		assertEquals(defaultLoggerRepository.getEffectiveLevel(logger), Level.INFO);
	}
}
